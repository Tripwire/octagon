...
const BannerMessage = props => {
  ...
  function getStatus () {
	...
	function getOneCloseElement() {
		const iconNotBool = icon && typeof icon != "boolean" && icon != "true"
		const iconCN = iconNotBool ? icon : "ei icon_close"
		const closeIcon= <i className={`banner-message__close ${iconCN}`} onClick={props.onCloseClicked}></i>
		const closeButton = <Button className="banner-message__close" onClick={props.onCloseClicked}>{props.button}</Button>
		return !icon && props.button && closeButton || closeIcon
	}
	...
	const alertIcon = <i className={`banner-message__alert ${getAlertIconClass()}`}></i>
	
	const closeElement = getOneCloseElement()
  return (
